...
public class PostProvider {
    ...
    public LiveData<String> addPost(Post post) {
        ...
        if(post.getPuntuacion()>100) {
            post.put("puntuacion", 100);
        } else {
            post.put("puntuacion", post.getPuntuacion());
        }
        post.put("categoria", post.getCategoria());
        post.put("precio", post.getPrecio());
        ...
        query.findInBackground((posts, e) -> {
            if (e == null) {
                ...
                for (ParseObject postObject : posts) {
                    ...
                    post.setPuntuacion(postObject.getInt("puntuacion"));
                    post.setCategoria(postObject.getString("categoria"));
                    post.setPrecio(postObject.getDouble("precio"));
    ...
    public LiveData<List<Post>> getTopPosts() {
        MutableLiveData<List<Post>> result = new MutableLiveData<>();
        ParseQuery<Post> query = ParseQuery.getQuery(Post.class);
        query.whereGreaterThanOrEqualTo("puntuacion", 85);
        query.include("user");
        ...
        query.findInBackground((posts, e) -> {
            if (e == null) {
                List<Post> postList = new ArrayList<>();
                for (ParseObject postObject : posts) {
                    Log.d("PostObject", "ID: " + postObject.getObjectId() + ", Título: " + postObject.getString("titulo") + ", Puntuación: " + postObject.getDouble("puntuacion"));
                ...
                result.setValue(new ArrayList<>());
                Log.e("ParseError", "Error al recuperar los top posts: ", e);
            }
        });
